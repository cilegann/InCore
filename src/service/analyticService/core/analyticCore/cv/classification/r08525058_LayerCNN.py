...
class r08525058_LayerCNN(classification):
    def trainAlgo(self):
        self.model=Sequential()
        self.model.add(Conv2D(self.param['hidden_neuron'],
                              (self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),
                              input_shape=(32,32,3),
                              data_format='channels_last',
                              activation=self.param['hidden_activation'],
                              strides = (self.param['kernelSize_stride'],self.param['kernelSize_stride'])
                             )
                      )
        self.model.add(Conv2D(self.param['hidden_neuron'],
                              (self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),
