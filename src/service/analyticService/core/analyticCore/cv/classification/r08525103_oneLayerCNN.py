...
class r08525103_oneLayerCNN(classification):
    def trainAlgo(self):
        self.model=Sequential()
        
        self.model.add(Conv2D(self.param['hidden_neuron'],(self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),input_shape=(32,32,3),data_format='channels_last',activation=self.param['hidden_activation'],padding='same'))
        ...
        self.model.add(MaxPooling2D(pool_size=(self.param['pool_size'], self.param['pool_size']),strides=2))
        self.model.add(Dropout(self.param['dropout']))
       
        self.model.add(Conv2D(2*self.param['hidden_neuron'],(self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),activation=self.param['hidden_activation'],padding='same'))
        self.model.add(Conv2D(2*self.param['hidden_neuron'],(self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),activation=self.param['hidden_activation'],padding='same'))
        self.model.add(MaxPooling2D(pool_size=(self.param['pool_size'], self.param['pool_size']),strides=2))
        self.model.add(Dropout(self.param['dropout']))
       
        self.model.add(Conv2D(4*self.param['hidden_neuron'],(self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),activation=self.param['hidden_activation'],padding='same'))
        self.model.add(Conv2D(4*self.param['hidden_neuron'],(self.param['hidden_kernel_size'],self.param['hidden_kernel_size']),activation=self.param['hidden_activation'],padding='same'))
        self.model.add(MaxPooling2D(pool_size=(self.param['pool_size'], self.param['pool_size']),strides=2))
        self.model.add(Dropout(self.param['dropout']))
